...
export default class BambuDriver extends OAuth2Device {
  ...
  async setFanSpeed({
    speed = BambuUtil.FanSpeed.SILENT, // TODO
  }) {
    if (!this.mqtt) {
      throw new Error('Not Connected');
    }
  ...
  async onState(state) {
    ...
    // Fan Speed
    const fanSpeed = state?.print?.fan_speed;
    if (typeof fanSpeed === 'number') {
      if (!this.hasCapability('bambu_string.fan_speed')) {
        await this.addCapability('bambu_string.fan_speed');
        await this.setCapabilityOptions('bambu_string.fan_speed', {
          title: 'Fan Speed',
        });
      }

      await this.setCapabilityValue('bambu_string.fan_speed', fanSpeed)
        .catch(this.error);
    }
  }
